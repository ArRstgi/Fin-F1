...
I have stored the data as a vector database due to the speed with which data can be retrieved from them. For embedding my PDF document, I have not used OpenAI's text embeddings since it can be done with a simple open-source, hugging face model. This reduces cost through fewer LLM calls and accomplishes the goal equally well. I have used the all-MiniLM-L6-v2 model from Hugging Face for this purpose.
...
In Map Reduce, every document/chunk of data is sent to the LLM and each of them gets their own answer to the query asked by the user. Then, all these answers are combined into one final answer. Again, similar to Contextual Compression,  this is a powerful technique but it involves many LLM calls
...
I went with the Refine technique since it involves fewer LLM calls while still being reasonably powerful.
...
Here, I simply used the ConversationBufferMemory object from LangChain to create an object that stores chat history. Then, when creating the ConversationalRetrievalChain, I passed this object for the memory parameter. This allows the LLM to keep track of the last 5 conversations with the chatbot, adding a degree of chat history to the chatbot.
...
I built a streamlit application that takes in the user's queries and passes them through the RAG pipeline before displaying the response.
...
1. The Chatbot struggles to follow up on answers when asked questions such as "why", "what" and similar questions that do not have specific details that can be searched for.
...
6. Further advanced retrieval techniques such as Small-to-Big Retrieval in terms of Parent Document Retrieval or Sentence Window Retrieval could be implemented to increase the chatbot's capabilities.
